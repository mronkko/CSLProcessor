...
-(id) initWithCSLFile:(NSString*)cslPath localeFile:(NSString *)localePath fieldMapFile:(NSString *)fieldMapPath{
    ...
    if(fieldMapPath!=NULL){
        ...
        if(![[NSFileManager defaultManager] fileExistsAtPath:fieldMapPath]){
            [NSException raise:@"File not found" format:@"CSL field map file not found: %@",fieldMapPath];
        }
        
        NSData* fieldMapData   = [NSData dataWithContentsOfFile: fieldMapPath];
        ...
        for(CXMLElement* element in [fieldMapDoc nodesForXPath:@"//typeMap" error:NULL]){
...
-(NSString*) formatBibliographyItemUsingVariables:(NSDictionary*)variables storeMacrosInDictionary:(NSMutableDictionary*) macros{
    ...
    for(NSString* key in variables){
        ...
        if(newKeys != NULL){
            for(NSString* newKey in newKeys){
                ...
                if(value==NULL){
                    NSLog(@"CSL formatter error: no value for key %@",newKey);
                }
                else{
    ...
    if([renderingDict count]==0){
        [NSException raise:@"No data" format:@"CSL formatter was called with empty dictionary"];
    }
